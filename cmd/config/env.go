...
// FORGE_MIRROR_VERSION, which is why one that is not recognized is reported rather
// than refused.
const EnvPrefix = "FORGE_MIRROR_"
...
// after the path to it, so `dsn` of `db` is read from FORGE_MIRROR_DB_DSN.
//
